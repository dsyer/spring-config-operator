...
func main() {
	...
	if err = controllers.ConfigClientReconciler(
		reconcilers.Config{
			Client:    mgr.GetClient(),
			APIReader: mgr.GetAPIReader(),
			Recorder:  mgr.GetEventRecorderFor("ConfigClient"),
			Log:       ctrl.Log.WithName("controllers").WithName("ConfigClient"),
			Scheme:    mgr.GetScheme(),
			Tracker:   tracker.New(syncPeriod, ctrl.Log.WithName("controllers").WithName("ConfigClient").WithName("tracker")),
		},
	).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ConfigClient")
		os.Exit(1)
	}
...
func createRecorder(log logr.Logger, mgr ctrl.Manager) record.EventRecorder {
	...
	recorder := eventBroadcaster.NewRecorder(clientgoscheme.Scheme, corev1.EventSource{Component: "spring-cpnfig-operator"})
	return recorder
}
